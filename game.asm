...
.extrn update_ampdisplay:proc
.extrn play_dmc_sample:proc

.extrn game_ui_data:label
.extrn pad3d_data:label
...
.proc go_to_title_screen
    lda #1 : sta main_cycle
    lda #0 : jmp swap_bank
.endp

.proc go_to_game_type_select
    lda #24 : sta main_cycle
    lda #6 : jmp swap_bank
.endp
...
.proc maybe_seek_one_target_pattern
    lda target_song_order_skip
    beq +
    dec target_song_order_skip
    lda #0 ; seek one pattern
    jsr sequencer_seek_order_relative
  + rts
.endp
...
.proc maybe_seek_to_target_pattern
    ...
    cmp target_song
    bne +
    jsr maybe_seek_one_target_pattern
    jsr maybe_seek_one_target_pattern
    jsr maybe_seek_one_target_pattern
    jsr maybe_seek_one_target_pattern
    jsr maybe_seek_one_target_pattern
...
.proc maybe_begin_song
    ...
    bne +
    rts
  + jsr maybe_seek_to_target_pattern
...
.proc wipeout
    ...
    tay
    jsr set_scroll_xy
    lda #(PPU_CTRL0_SPRITE_SIZE_8x16 | PPU_CTRL0_BG_TABLE_0000 | PPU_CTRL0_SPRITE_TABLE_1000)
    sta ppu.ctrl0
    lda #(PPU_CTRL1_BG_CLIP_ON | PPU_CTRL1_SPRITE_CLIP_ON)
    sta ppu.ctrl1
.ifdef MMC
.if MMC == 3
    lda #MMC3_MIRROR_V
...
; D-PAD HERO (letter placeholders)
...
; progress indicator (initially "empty")
.db $20,$6B,$54,$0F
; energy bar (initially full)
.db $20,$8B,$54,$0A
.ifdef LIFE_SUPPORT
; the X left of life count
.db $20,$67,$01,$1F
.endif
; attributes
.db $23,$C3,$45,$AA
...
; initialize the target
    ...
    asl : asl : asl
    sta targets_2.speed_x.frac,x ; temp
    tya
    ora targets_2.speed_x.frac,x
    tay
    lda @@speed_x_lo,y
...
; LEFT, RIGHT, SELECT, B, A
    ...
    @@speed_x_hi:
    ; level 0
    .db $FF,$FF,$00,$00,$00 : .db 0,0,0
    ; level 1
    .db $FF,$FF,$00,$00,$00 : .db 0,0,0
    ; level 2
    .db $FF,$FF,$00,$00,$01 : .db 0,0,0
    ; level 3
    .db $FE,$FF,$00,$00,$01 : .db 0,0,0
    ; level 4
    .db $FE,$FF,$00,$00,$01 : .db 0,0,0
    ; level 5
    .db $FE,$FF,$00,$00,$01 : .db 0,0,0
    ; level 6
    .db $FE,$FF,$00,$01,$02 : .db 0,0,0
    ; level 7
    .db $FD,$FE,$00,$01,$02 : .db 0,0,0
    @@speed_x_lo:
    ; level 0
    .db $80,$C0,$00,$40,$80 : .db 0,0,0
    ; level 1
    .db $40,$A0,$00,$60,$C0 : .db 0,0,0
    ; level 2
    .db $00,$80,$00,$80,$00 : .db 0,0,0
    ; level 3
    .db $C0,$60,$00,$A0,$40 : .db 0,0,0
    ; level 4
    .db $80,$40,$00,$C0,$80 : .db 0,0,0
    ; level 5
    .db $40,$20,$00,$E0,$C0 : .db 0,0,0
    ; level 6
    .db $00,$00,$00,$00,$00 : .db 0,0,0
    ; level 7
    .db $C0,$E0,$00,$20,$40 : .db 0,0,0
...
.proc maybe_request_lane_switch
    ...
    beq +
    ; 2-player
    lda lane_switcher_offset
    cmp #$FF
    bne + ; don't request lane switch if there is already a switcher
    lda #1
    sta lane_switch_request
  + rts
.endp
...
; left half
  ...
  + asl
    asl ; type * 32
    sta sprites.tile,x
    lda targets_1.pos_y.int,y ; sprite ("size") is determined by Y
    sec
    sbc #48 ; initial Y pos
    lsr
    lsr
    and #$FC
    cmp #$20
 ...
 ++ sta sprites.tile,x
  ...
  + lda targets_1.state,y
    and #7 ; lane
    tay
    lda normal_target_attributes,y
 ++ sta sprites.attr,x
    pla
    tay
; right half
...
.proc update_palette_for_time_mode
    ...
    beq + ; only if there is no fading in progress
    rts
  + lda transition_timer
    ...
    rts
  + lda time_mode
    bne @@fade_towards_green

    ; fade towards game palette
    ldy #15
  - lda palette,y
    cmp game_palette,y
    beq +
